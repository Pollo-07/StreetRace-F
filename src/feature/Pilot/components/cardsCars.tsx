...
const CardsCars = ({ active,setOpen,setMode,setDataEdit}: Props) => {
  ...
  const carImage =
    ...
    const {activeVehicle,deleteVehicle} = UseVehicle()
      ...
      const  vehicles = UseVehicle().vehicles 
       const vehiculoFilter = active ? vehicles?.filter((vehiculo) => vehiculo.activo === true): vehicles
  ...
  return (
    ...
    {
      !vehiculoFilter || vehiculoFilter.length === 0 ? <p style={{color:"white"}}>no hay vehiculos</p>
      :(
          vehiculoFilter?.map((vehiculo,index)=>{
            ...
            <Card key={index}
                  ...
                  sx={{
                    ...
                    border: vehiculo.activo? "":"1px solid rgba(255,255,255,0.2)",
                    borderRadius: 2,
                    cursor: "pointer",
                    color: "white",
                    "&:hover": {
                      boxShadow: vehiculo.activo? " 0 0 12px rgb(0, 255, 132)": "0 0 12px #00f0ff",
                    },
                    overflow: "hidden",
                    position: "relative",
                    mt:5,
                    boxShadow: vehiculo.activo?" 0 5px 12px rgba(0, 230, 118, 0.4)":"",
                  }}
                  ...
                  <Box sx={{ flex: 1, position: "relative", overflow: "hidden" }}>
...

                    {
                      vehiculo.activo?    <Box
                               ...
                               <Tooltip title="Eliminar vehiculo" sx={{backgroundColor:"red"}}>
                                     <CloseIcon
                  ...
                  >
                    ...
                    <Box>
                      ...
                      <Box sx={{ display: "flex", gap: 2, mt: 1 }}>
                        ...
                        <Box sx={{ backgroundColor: "#1c1b1b", p: 1, borderRadius: 5}}>
                          <Typography variant="caption">{vehiculo.modificaciones? vehiculo.modificaciones:"no disponible"}</Typography>
                        </Box>
                       ...
                       {!vehiculo.activo && 
                            <Button
                            onClick={()=>activarVehiculo(vehiculo.id)}
                      variant="contained"
                      sx={{
                        fontSize: 15,
                        backgroundColor: "#104153",
