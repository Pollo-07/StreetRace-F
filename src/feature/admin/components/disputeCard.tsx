...
export const DisputeCard = () => {
  ...
  return (
    <Box>
      {challengeDisputed && challengeDisputed.length > 0 ? (
          ...
          >
            ...
            >
              ...
              <Typography sx={{ fontSize: 10, color: "#888" }}>
                {new Date(item.challenge.fecha_acordada).toLocaleDateString(
                  "es-CO",
                )}
              </Typography>
            </Box>
        ...
        >
          no hay challenge en disputada
        </Typography>
      )}

      {Select && (
        <DisputeOpinionModal open={open} Setopen={Setopen} challenge={Select} />
      )}
    </Box>
  );
};
